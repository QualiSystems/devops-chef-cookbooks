linux_headers_pkgs = ChefRubyHelper.get_packages_containing("'linux-headers'")
package linux_headers_pkgs do
  action :purge  
end

redundant_linux_kernels = ChefRubyHelper.get_packages_containing("'linux-image-.*-generic'", "`uname -r`")
package redundant_linux_kernels do
  action :purge
end

linux_source = ChefRubyHelper.get_packages_containing("'linux-source'")
package linux_source do
  action :purge
end

doc_packages = ChefRubyHelper.get_packages_containing("'-doc$'")
